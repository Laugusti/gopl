...
func mapToPlane(p, units, min, max int) *big.Rat {
	result := big.NewRat(int64(p), int64(units))
	result.Mul(result, big.NewRat(int64(max-min), 1))
	result.Add(result, big.NewRat(int64(min), 1))
	return result
}
